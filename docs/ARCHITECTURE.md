...
1. **A typed MCP server surface** (11 forensic functions, no `execute_shell`) so the agent physically cannot run destructive commands.
...
3. **A verifier-gated promotion path** (Witness Oath Verifier + Ralph Wiggum Loop) so an LLM claim becomes a finding only if a deterministic re-derivation of the cited evidence agrees.

Concretely, both install paths (`scripts/install-on-sift.sh` for the SIFT Workstation, `scripts/install-tools.sh` for macOS) first run Protocol SIFT's own installer (`curl -fsSL https://raw.githubusercontent.com/teamdfir/protocol-sift/main/install.sh | bash`) and then layer OATH's typed-function server + verifier on top. Skip with `OATH_SKIP_PROTOCOL_SIFT=1` if you've already installed it.
